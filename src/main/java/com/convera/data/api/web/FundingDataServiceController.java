...
public class FundingDataServiceController {
  ...
  public ResponseEntity<CommonResponse<Order>> postTestInsert(
    ...
    try {
      ...
      return ResponseEntity.ok().body(CommonResponseUtil.createResponse200(order, "/convera/funding/orders", correlationId));
    } catch (Exception ex) {
      log.error("Exception while saving to database", ex);
      return ResponseEntity.internalServerError()
          .body(CommonResponseUtil.createResponse500(null, "/convera/funding/orders", correlationId, Collections.singletonList(
  ...
  public ResponseEntity<CommonResponse<Set<ContractFunding>>> createContractsFunding(
    ...
    try {
      ...
      return ResponseEntity.ok()
          .body(CommonResponseUtil.createResponse200(contractFundingSet, "/contractsFunding/all", correlationId));
    } catch (Exception ex) {
      log.error("Exception while saving to database", ex);
      return ResponseEntity.internalServerError()
